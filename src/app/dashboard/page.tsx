...
import { GoNextIcon } from '../components/common/Icons';
...
import {
  ...
  dashboard_User_Engagement_data,
  progressData,
  Totalwork_data_dashboard,
      ...
      <div className="overflow-y-auto">
        <div className="flex flex-wrap lg:mt-2 ">
          {Totalwork_data_dashboard.map((items, index) => (
            <div key={index} className="w-full sm:w-1/2 lg:w-3/12 px-2.5 xl:px-2 2xl:px-3 mt-5">
              <div className="flex justify-between px-3 2xl:px-6 py-4 rounded-2xl bg-white custom_shadow">
                <div>
                  <p className="font-bold text-xs text-[#9A9999] pb-1">{items.progress}</p>
                  <h3 className="font-semibold text-xl md:text-2xl lg:text-[28px] text-[#213737]">
                    {items.totalwork}
                    <span className="text-sm text-[#477D7C] ms-1">{items.taskname}</span>
                  </h3>
                </div>
                <div>{items.icons}</div>
              </div>
        ...
        <div className="flex flex-wrap mt-6 ">
              ...
              <div className="w-full xl:w-7/12">
                <div className="h-full flex flex-col justify-between">
                  <div className="pe-5 2xl:pe-24">
                    <p className="font-bold text-xs text-[#9A9999] pb-1">11:20 2/23/2025</p>
                    ...
                    <p className="font-normal text-sm text-[#9A9999] ff_helvetica">
                      Your projects are ready for growth, with resources to tackle larger
                      challenges.
                    </p>
                  </div>
                  <Link
          ...
          <div className="w-full xl:w-1/2 2xl:w-5/12 px-2.5 xl:px-3 mt-6 xl:mt-0">
            <div className="px-3 2xl:px-6 py-4 rounded-2xl bg-white custom_shadow h-full relative">
              ...
              <Image className="w-full h-full rounded-[12px]" src={csatScoreImg} alt="CSAT-SCORE" />
            </div>
          </div>
        </div>
        <div className="flex flex-wrap mt-6 ">
          <div className="w-full lg:w-1/2 2xl:w-4/12 px-2.5 xl:px-3">
            <div className="px-3 2xl:px-6 py-4 rounded-2xl bg-white custom_shadow h-full">
              ...
              <p className="font-bold text-sm text-[#9A9999] leading-[140%] ff_helvetica mt-1.5">
                <span className="text-[#213737] font-normal">180 </span> Completed tasks
              </p>
              <p className="font-normal text-sm text-[#333] py-6 text-center">Percent complete</p>
              <div className="flex flex-col gap-[58px] mb-6">
                {progressData.map((item, index) => (
                  <div key={index} className="flex items-center gap-4">
                    <p className="text-gray-700 text-sm font-medium w-24">{item.label}</p>
              ...
              <div className="flex pe-10 mb-6 justify-between h-[calc(100%-70px)]">
                {completionData.map((item, index) => (
                  <div key={index} className="flex flex-col justify-end items-center  h-full">
                    {/* Percentage */}
                    <p className="text-[#333333] font-bold mb-1">{item.value}%</p>
                  ...
                  {dashboard_User_Engagement_data.map((item, index) => (
                    <div key={index} className="w-[22%]">
                      <p className="text-xs font-bold text-[#9A9999]  flex gap-1 items-center">
                        {' '}
                        <span className="block w-3 h-3  rounded-full bg-[#477D7C]"></span> Task #
                        {item.id}
                      </p>

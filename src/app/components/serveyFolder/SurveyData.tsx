...
const SurveyData: React.FC<SurveyDataProps> = ({ setIsModalOpen }) => {
  ...
  return (
    <section
      className={`${inter.className} overflow-y-scroll h-[74vh] pe-6`}
    >
                ...
                <div className="mb-4 w-full relative">
                  ...
                  {openDropdownIndex === index && (
                    <motion.ul
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute w-full mt-2 bg-white border border-[#D6D6D6] rounded-2xl shadow-lg z-10 overflow-hidden"
                    >
                      {options.map((option) => (
                        <li
                          key={option.id}

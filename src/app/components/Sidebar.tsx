...
const Sidebar: React.FC<SidebarProps> = () => {
  ...
  return (
    <div id="sidebar" className="top-0 sticky">
      ...
      <div className="flex flex-col gap-2 h-full mt-[50px]">
        {sideBarData?.slice(0, 4)?.map((item: SidebarItem, index: number) => (
          <div key={index}>
            <Link
              href={`${item.path}`}
              className={`flex items-center justify-between px-5 py-3 gap-2.5 hover:bg-[#fff] rounded-2xl hover:custom_shadow w-full duration-300 icon cursor-pointer group 
      ...
      <div className="p-4 rounded-[12px] bg-[#2C4C4B] mt-16 bg-cover bg-center bg-no-repeat max-md:hidden">
        <QuestionIcon />
        <h3
